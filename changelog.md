...
### Added
- Added an option called `professionCosts` which is used to specify the costs for each villager individually, following this format: profession_id=COST_TYPE:cost_value;amount

### Bug Fixes

- Fixed a rare crash when the mod tried to use a translation from another mod while the `useModTranslations` setting was enabled

### Removed

- `generateVillagerAttempts` was removed as it was no longer used in the code thanks to optimizations, as I plan to redesign the mod's configuration
...
- feat: New `enableCooldown` option to configure the cooldown time to prevent players from changing trades until they get the desired one villager also display messages when they are tired due to this cooldown (Currently there are only a few, but more will be added over time)
- feat: New `autoNameVillagers` option for villager names it simply helps make the messages make more sense, but it's still in development and isn't finished yet (though I recommend using the Villager Names Mod created by Serilum instead, since it does a better job with this feature)
...
- feat(config)!: Added `contractCostType` and `contractCostAmount` to the configuration file
